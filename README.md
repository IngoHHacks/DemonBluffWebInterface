...
The puzzles were generated by modifying the `StateExporter` mod to automatically reset, perform actions, and export the state after all characters were revealed.  
Due to being generated with random actions, these may not be fully representative of real game situations, but they serve well for validation and benchmarking purposes.  
The WebInterface solver can solve all 1000 puzzles correctly. When contributing a new solver or improving the existing one, please make sure it can solve all 1000 puzzles correctly. The `Validate` scene and script should help with that.
